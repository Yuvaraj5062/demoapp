...
const CreateVoucher = () => {
  ...
  const onCreateNewVoucher = () => {
    ...
    if (!discountType.value || discountType.value === "") {
      setDiscountType({
        ...discountType,
        isError: true,
    ...
    if (title.value === '') {
      ...
      isOK = false;
    } else if(discount.value === '' && discountType.value === "InPercent"){
      setAlertStatus({
        status: true,
      ...
      isOK = false;
    }else if(discount.value === '' && discountType.value === "InAmount"){
      setAlertStatus({
        status: true,
        message: `${t(`Please select discount amount`)}`,
        type: "error",
      })
      isOK = false;
    }
    ...
    if (isOK) {
      ...
      setLoader(true);
      dispatch(actions.createVoucher(body))
        .then((response) => {
          if (response === "Success") {
            setAlertStatus({
              status: true,
              message: `${t(`Voucher created successfully`)}`,
              type: "success",
            });
            setLoader(false);
            setTimeout(function() {
            ...
            <div className="col-md-8">
              <Form.Group controlId="pointsBasic">
                ...
                <div class="custom-file ">
                  ...
                  >
                    {imageFile.value ? imageFile.value : `${t(`Choose file`)}`}
                  </label>
                </div>
              </Form.Group>
            </div>
